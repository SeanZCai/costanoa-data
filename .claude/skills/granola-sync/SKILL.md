...
- Env file (per user): `~/.costanoa-data/.env`
...
## Step 0 — First-run bootstrap (auto)

Before anything else, run this check:

```bash
test -f "$HOME/.costanoa-data/.env" && test -d "$HOME/.costanoa-data/.venv" && echo OK || echo BOOTSTRAP
```

If the output is `BOOTSTRAP`, the teammate hasn't been set up yet. Walk them through it:

1. Ask the user for their `SUPABASE_URL` (e.g. `https://lgnejbduwpytgbvtnhpr.supabase.co`). Sean shares this in the team password manager.
2. Ask for their `SUPABASE_SERVICE_ROLE_KEY` (a long token). Also from the password manager.
3. Confirm before writing — show the URL back to them and ask "OK to save?" Don't echo the key.
4. Once confirmed, run:

```bash
mkdir -p "$HOME/.costanoa-data"
cat > "$HOME/.costanoa-data/.env" <<EOF
SUPABASE_URL=<paste from step 1>
SUPABASE_SERVICE_ROLE_KEY=<paste from step 2>
TEAM_DOMAINS=costanoa.vc,costanoavc.com
EXCLUDE_PHRASES=
EOF
chmod 600 "$HOME/.costanoa-data/.env"
...
Report success and proceed to Step 1. If the bootstrap check returned `OK`, skip this entire step.
